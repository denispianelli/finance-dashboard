...
**Related ADR:** [ADR-012 — Learned account routing for multi-file import](../../adr/012-account-identifier-routing.md) (Proposed)
...
- IBAN ↔ OFX `ACCTID` reconciliation (ADR-012 — per-source keys).
...
### 4.1 Migration `008_account_identifiers.sql`
...
6. `ImportModal`: drop zone, ChooseAccountView, ReviewView tweaks, SummaryView
   (+ component tests).
7. Integration test with LCL fixtures; manual smoke; ADR-012 → Accepted.
...
- Lint clean, `tsc --noEmit` clean, unit tests green, `npm run build` succeeds
  (CLAUDE.md DoD). ADR-012 promoted to Accepted.
